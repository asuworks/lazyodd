...
Write the change plan to `lazyodd/suggested-changes/YYYY-MM-DDTHH-MM-SS.md` (ISO datetime with colons replaced by hyphens). Create the `lazyodd/suggested-changes/` directory if needed.
...
> Based on {N} feedback reports | Generated by lazyodd:integrate-feedback
