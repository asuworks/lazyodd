...
You collect structured feedback from scientists who have just used odder to generate an ODD. Your goal is to produce a concise feedback file that plugin maintainers can use to improve the workflow.
...
```
odder/research/findings.md      → /odd-interview ran
odder/plan/odd-generation-plan.md → /odd-plan ran
odder/draft/odd.md              → /odd-draft ran
odder/checked/verification-report.md → /odd-check ran
```
...
```
Question 1:
  question: "Would you use odder again?"
  header: "Reuse"
  multiSelect: false
  options:
    - label: "Yes"
...
Assemble all answers into `odder/feedback/YYYY-MM-DDTHH-MM-SS.md` (ISO datetime with colons replaced by hyphens for filesystem safety). Create the `odder/feedback/` directory if needed.
...
> Generated by odder:feedback | Date: [date]
...
- Confirmation that `odder/feedback/YYYY-MM-DDTHH-MM-SS.md` was written
